...
Qualitative observations only (no fabricated percentages or throughput numbers):
- **Statefulness**: where state lives, whether services can be replicated horizontally
- **Session / auth**: in-memory vs. externalized
- **Database connections**: pooling, connection limits, read/write split
- **Caching**: what's cached, where, invalidation strategy
- **Hot paths**: CPU/I/O-bound operations identified in code (with file:line)

### Technical Debt Analysis
List concrete debt items grounded in the codebase. For each:
- **Location**: file or module
- **Nature of debt**: what's wrong structurally
- **Risk if not addressed**: High / Medium / Low
- Do NOT estimate effort in sprints/days — you lack team velocity context.
...
- Ground these in actual issues found in the codebase (not generic buzzword suggestions)
- Each recommendation should cite the file(s)/module(s) it applies to
```
